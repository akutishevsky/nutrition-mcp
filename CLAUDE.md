...
In-chat UI uses **MCP Apps** (the official 2026-01-26 MCP extension), which renders across Claude, ChatGPT, VS Code, Goose, and MCP Inspector from one implementation. Widgets are **assembled from source partials at server startup** — not committed as built files. Sources live in `public/widgets/src/` (`shared/` partials + one `templates/*.html` per widget); `src/widgets.ts` inlines the partials (resolving `/*@include shared/…@*/` markers) into one self-contained HTML string per widget, cached and warmed at boot (`warmWidgets()` in `src/index.ts`). `src/mcp.ts` serves each via `getWidgetHtml(key)`. Every in-chat widget is **one Dawn card** — the public site's design language (`public/styles.css`) ported into a chat card, same token names: a header line, the widget's own top matter (a chart, a range toggle), then the shared macro strip — a calorie **hero** (an SVG gauge beside its figure and a delta pill), **one** wrapping **chip rail** (the macros, water, the "tap a metric" hint chip, and any limit currently breached), a `.more` disclosure row naming the limits it hides (sugar / alcohol / caffeine / fiber) with those chips on a second rail behind it, and the single shared **drawer** they all open into. A chip carries the metric's name, its value and a 2.5px progress underbar; the goal, the distance to it and the meals behind it are one tap away, which is what let the old per-nutrient bars and always-visible captions go. A breached limit is **hoisted back onto the visible rail**, so an alarm is never one tap away — collapsing the limits is only ever hiding metrics that are fine. The shell lives in `shared/base.css` (`.wrap` — which **is** `#root` — `.card`, `.glow`, `.chead`/`.ctitle`/`.cmeta`/`.csub`, `.sec`, `.empty`, the `.c-*` nutrient role classes that set `--c`), the interaction primitives in `shared/chip.css` (`.rail`, `.chip`, `.hero`/`.gauge`, `.more`, `.drawer`, `.seg`), the charts in `shared/chart.css` + `shared/svg.js` (geometry as pure string math), the icons in `shared/icon.js` (`icon(name, size)` over a nine-path table), and the strip's markup in `shared/macros.js`. The widgets: the `get_nutrition_summary` dashboard, the `get_goal_progress` view, the meal-progress strip (`meal-logged`, which renders nothing when no goals are set), the `get_trends` view (interactive 7/14/30-day toggle), the `get_weight_trends` view (data-scaled weight-over-time chart with the same toggle), and `import-meals` (the bulk-import flow — see "Bulk meal import" below). `component-gallery` is dev-only: it is listed in `WIDGET_TEMPLATES` so it is assembled and test-covered, but no `ui://` resource or tool references it, so no client can reach it — view it with `bun run harness`. They share one design language and one host bridge — see `public/widgets/STYLE_GUIDE.md`. `bun test src/widgets.test.ts` guards assembly (no unresolved markers, valid inline JS, every partial inlined in full).

A second marker, `/*@inlinets src/csv.ts@*/`, transpiles a TypeScript module from `src/` into the widget as plain JS. It exists so a widget can run **tested** server-side code instead of a hand-copied twin; it only works for modules with no runtime imports, and it strips module syntax because a bare `export` inside a `<script>` is a syntax error. `import-meals` uses it twice — `src/csv.ts` (the parser) and `src/chunk.ts` (the per-call row chunker).
...
`shared/i18n.js` resolves which locale to render, exposed as the ambient `T` (mirroring `data-theme` as ambient theme rather than a threaded argument): `pickLocale(explicit, hostLocale)` prefers `structuredContent.locale` — per-user, always set — over the host's `hostContext.locale` — host-dependent, may be absent — over `"en"`, matched on BCP-47 base language. A template calls `setLocale()` from its `render()` once the payload's locale is known. `WidgetStrings` is plain strings except `PluralForms` (`{ one, other, few?, many? }`), picked via `Intl.PluralRules` — a browser built-in, no network request, so it's fine under the iframe's `default-src 'none'` CSP — falling back to `other` for any category the data lacks, consistent with translation here being AI-generated with no human review pass (same as the public site's — see below). Ukrainian and Polish **must** carry `few`/`many` on every plural key (`src/widgets.test.ts` pins it): with only `one`/`other` they printed "3 днів" for "3 дні". Calendar dates go through `Intl.DateTimeFormat` in `shared/date.js` (`intlDateRange`, `shortDate`), with English pinned to `en-GB` ("5 Jul") and a guard for engines whose `formatRange` falls back to numeric output (ja, pl on Safari).

**Every widget except `component-gallery` (dev-only, unreachable by any client) is fully wired end-to-end.** `nutrition-summary`, `goal-progress`, `meal-logged`, `trends`, `weight-trends`, and `import-meals` each call `setLocale(pickLocale(structuredContent.locale, hostContext.locale))` from `render()` (or, for `import-meals`, from the `initWidget` config's outer `render(data)` callback, since its own internal `render()` repaints steps without fresh data) and have their own `WidgetStrings` namespace (`goalProgress`, `mealLogged`, `trends`, `weightTrends`, `importMeals`) alongside `macros`/`nutritionSummary`, translated into every locale. The shared `macros` namespace gained `mealTypes` (`breakfast`/`lunch`/`dinner`/`snack`) for the same reason: `meal_type` was reaching meal-logged's `.csub` straight out of the payload, so a fully translated Japanese card read "7月15日 · **lunch** · …" — and the layout round had just moved the type to the front of that line. `mealTypeLabel()` looks it up case-insensitively and **falls back to the raw stored string**, because those four are only the current server enum and an older row must still show its own word. `weight-trends` and `import-meals` didn't carry the `@i18n` machinery at all until this was added — they now `@include shared/i18n.js` like the others. Every tool whose widget needs it (`get_goal_progress`, `log_meal`/`update_meal` via `buildMealProgress`, `get_trends`, `get_weight_trends`, `start_meal_import`) resolves `locale` via `getUserLocale`/`localeFromProfile` and includes it in `structuredContent`, mirroring `get_nutrition_summary`'s original field. Two deliberate non-translations, not gaps: `import-meals`'s `diagnosticsBlock()` copy-paste support-email dump stays English (addressed to the English-speaking maintainer, not the widget's audience), and `api.updateModelContext`'s finished-import summary stays English (model-facing text like every tool's `content`, not UI) — see the doc comment on `importMeals` in `src/copy/widgets.ts`. Extending a widget further is: call `setLocale()` from `render()`/`onReady()` as these do, and add its strings to `WidgetStrings` (every locale file) alongside the existing keys. One namespace sits **outside** every widget's: `chrome` (`widgetsNote`, `noHost`) is the text `shared/bridge.js` itself paints around whatever a template renders — the bridge does not know which widget it is hosting, and `noHost` replaces that widget's output entirely. It is read as `T.chrome.*` including on the failure path, before any template has called `setLocale()`, which works because `shared/i18n.js` initialises `T` to `WIDGET_STRINGS.en`.

**`bun run harness`** is the local host simulator (`scripts/widget-harness.ts`). It mimics a strict host — validates the `ui/initialize` shape, withholds the tool result until `ui/notifications/initialized`, starts the iframe at 130px, applies the sandbox CSP — and additionally answers app-initiated `tools/call`, sends host→app requests, and executes the real `bulk_import_meals` against an in-memory store. Query flags reproduce host behaviour: `?serverTools=0`, `?tools=0`, `?delay=3000`, `?maxHeight=600`, `?fail=1`, `?drinkUnit=us|uk`, `?locale=de` (any `WIDGET_STRINGS` code — overrides the fixture's `locale` and sets `hostContext.locale`; the host page links every language), `?waterUnit=us_fl_oz|uk_fl_oz|l` (overrides the fixtures' `water_unit`, to review the fluid-ounce reading without a pounds profile). Its `size-changed height=N` log line is the honest height measurement, which is why the harness is where layout is signed off — and why **each widget's canned fixture is validated against that tool's real `outputSchema` before the server starts** (`assertFixturesMatchSchemas`, which pulls the live schemas by registering `registerTools` against a throwaway `McpServer` and intercepting `registerTool`, so nothing is restated). Missing and stale fields both abort the harness. They had drifted: `trends` sent `range_days` where the tool sends `default_range` and `weight-trends` sent no `target`, so the seg opened on the wrong range and the dashed target line plus its whole `.cfoot` phrase never rendered here at all — a clipping bug in that phrase reached an audit instead of the harness. A fixture that exercises a fallback path is a review of the wrong thing.
...
**Styling — reuse the shared design language.** All widgets share one look (Dawn: soft neutral surfaces, brand green accent, theme tokens whose names match `public/styles.css`, big radii, one drifting `.glow` blob per card, one card per widget, an SVG gauge, chip underbars, hand-built SVG charts). Layout inside the strip is driven by each `MACROS` entry's `role` in `shared/macros.js` (`cal` / `macro` / `limit` / `bar`) — never by a hardcoded key list, so a new nutrient appears exactly where its role says and nowhere else; its **colour** likewise arrives as a `.c-*` role class setting `--c`, so no emitter in that file ever writes a colour and adding a nutrient is one `MACROS` entry, one token in `tokens.css`, one line in `base.css`. `macroPanel`'s `opts` carry the couplings: `chartKeys`/`onSeries` make a chip re-stroke the widget's chart (that is the whole of trends' interactivity, whose payload has no meals), `flatHero` drops the gauge, `divided` opens the strip with a `.sec` hairline. The tokens and component CSS live as source partials in `public/widgets/src/shared/` and are inlined via `@include`; **`public/widgets/STYLE_GUIDE.md`** is the spec for those partials. Edit a partial once and every widget picks it up on next assembly — do **not** re-inline or fork a shared block into a template. **Never animate a layout property** (`height`, `max-height`, `margin`, `padding`): the bridge measures `documentElement` at `max-content` on a `ResizeObserver` tick, so an animated size becomes a 60Hz `size-changed` storm — the drawer animates `opacity`/`transform` only, and nothing is ever made visible _by_ an animation. **A metric is read in one unit, decided in one place**: a `MACROS` entry may carry `display` (water's is the only one: litres, `{ unit: "L", per: 1000, decimals: 1 }`, or whole US/UK fluid ounces — each strip tool sends a `water_unit` resolved by `waterUnitFor` in `src/units.ts`, fluid ounces when the profile weighs in pounds, sized by the drink-unit preference, and the template's `render()` hands it to `setWaterUnit()`; nutrients stay in grams for everyone), and every renderer — chip figure, caption, accessible name, gauge label **and any chart foot a template draws** — goes through `macroNum(m, v)`/`macroUnit(m)`. Writing `fmt(x, m.decimals)` or interpolating `m.unit` instead is how the card ended up saying "2.1 L" on a chip and "avg 2,100 ml" two lines under it, twice, the second time being a WCAG 2.5.3 label-in-name failure as well. **A state is decided on the printed figure, never the raw float**: `macroBits` compares whole display steps (`macroSteps`), so 159.6 of 160 reads "160/160 g · at goal" rather than "0 g left", and 45.04 against a 45 g limit is "at limit", not red. And **a missing value is "none logged", never 0** — an absent `averages` key once rendered as a measured "0/160 g". Five a11y rules are load-bearing enough to repeat here (the rest are STYLE_GUIDE.md's invariants): a **series token is an identity, not text** — put `--cal`/`--pro`/… on a dot, fill, hairline or tint and keep small text in `--ink`/`--ink2`, because 10.5px in `--cal` measured 2.06:1; **`aria-expanded` means something expands**, so a control that only re-strokes the chart is `aria-pressed` and wears no chevron; **a disclosure hands focus both ways** — into the region on open, back to its trigger on close, before the contents holding focus are destroyed (and only when focus is actually inside it, or a cross-close steals focus from the control the user just used); **an over-a-ceiling state is never colour alone** — `.chip.over` swaps its dot for a `.wmark` warning triangle, so the breach survives greyscale and a hue everyone cannot separate; and **a scrollable box is a tab stop**, so it needs a name and the system's ring, not the UA's — `.tscroll` gets `outline: 2px solid var(--acc)` from `table.css` plus `role="region"` + `aria-label` + `tabindex` from the template that knows what is scrolling. Keep the JS host handshake in `shared/bridge.js` (`initWidget(config)`); a template supplies `{ name, loading, coerce, render, sample }`, plus an optional `onReady(api)` when it needs to call the server. `api` exposes `callTool(name, args, opts)`, `canCallTools` (whether the host advertised `hostCapabilities.serverTools`), `hostContext`, `hostInfo`, and `updateModelContext(text)`. Form controls and the preview table live in `shared/form.css` and `shared/table.css`; `.card` is the shared surface in `shared/base.css`.

Three bridge invariants that are easy to break: a message is only treated as a response to our request when it has **no** `method` and carries `result`/`error` (the host sends its own requests, with its own id counter — the spec's `ui/resource-teardown` example uses `id: 1`, which is why our ids are namespaced `app-N`); inbound messages are rejected unless `event.source === host` (`window.parent.frames` is reachable cross-origin, so a sibling iframe could otherwise forge a tool result and repaint a widget); and the footer note ("You can enable or disable these widgets anytime…") is **one** element kept as `#root`'s last child by a `MutationObserver` on the root's `childList`, not re-created after each `paint()` — several widgets repaint from their own controls (the trends and weight-trends range toggles) without going through `paint()` at all, and appending after `config.render()` alone lost the note on the first interaction. The observer removes it when the root has no children, which is what keeps `.wrap:empty { padding: 0 }` matching so `meal-logged` with no goals still collapses the iframe to zero height.
...
**The whole site is on one design, "Dawn"** (Claude Design `Landing 6a Dawn.dc.html` + the `design-system-v2.dc.html` system document): soft, rounded, glassy — Urbanist for everything that is not a figure, Geist Mono for eyebrows / URLs / tool names / clocks, big radii (18–36px), a floating glass pill header, three drifting colour blobs behind the hero, and the same nutrient palette (`--cal --pro --car --fat --wat --fib --sug --caf`) the in-chat widgets use. It was built for the landing page first and generalised outward in phase 2, which is why the shared classes are all `nm-*`. Light-mode `--acc` and `--ink3` deliberately differ from the design file for AA contrast — see the token block.

- **`public/styles.css`** is the whole system, served at `/styles.css` and loaded by every page through `HEAD_ASSETS` (Urbanist + Geist Mono from Google Fonts, Font Awesome 7.2.0 from cdn.jsdelivr.net, both already in the CSP). Tokens sit on `body`; the dark palette is written out twice (once under `prefers-color-scheme`, once under `body[data-theme="dark"]`), so a new colour has to land in all three blocks. The file reads top to bottom as tokens → keyframes → base → site.js hooks → blobs → colour roles → header / sheet menu → layout primitives, tiles, buttons → the landing-only sections (hero chat, how, connect, onboarding, examples, live, support, contact, FAQ, closing band, scoped `body.landing` where they must not leak) → footer → the **"Shared page primitives"** block near the end (page hero, glass pill, `.nm-crumb`, `.nm-tag`, `.nm-chip`/`.nm-chips` incl. `-scroll`/`-sticky`, `.nm-seg`, grids, `.nm-card`/`.nm-card-dark`, lists, `.nm-prose`, `.nm-toc`, `.nm-notice`, `.nm-field`/`.nm-input`/`.nm-label`, `.nm-frame`) that every non-landing page is built from → reduced motion. Layout only one page needs goes in that page's generator as an inline `<style>` scoped under its body class (`landing`, `tools`, `alt`, `legal`, `login`) using the shared tokens and classes — never a colour literal, never a fork of a shared block. Every class is `nm-`-prefixed except the hooks `site.js` keys off (`.skip`, `.vh`, `.tab-input`, `.head-nav`, `.lang-switch`, `.menu-btn`, `.site-menu`, `.nav-badge`, `.copy-mini`, `[data-reveal]`, `.translation-notice`, `.footer-links`), the rest of the header/sheet chrome (`.burger`, `.icon-btn`, `.lang-code`, `.lang-menu`, `.menu-label`, `.menu-secondary`, `.menu-foot`) and the login page's `.error-banner`, which `src/oauth.ts` emits at request time — `grep -oE '^\.[a-zA-Z][a-zA-Z0-9_-]*' public/styles.css | sort -u | grep -v '^\.nm-'` is the authoritative list.
- **`public/site.js`** (served at `/site.js`, loaded with `defer` by every page) owns the three-state theme (System is stored as the _absence_ of the `theme` key; `[data-theme-set]` buttons in the header's `.nm-theme[role=group]` segmented control stamp `data-theme` on `<body>`), the sticky header's reading progress (`--progress`, 0..1, which fills the pill bar itself with an accent wash — `.nm-bar`'s background gradient, not a hairline), the mobile sheet menu (`#menu-btn` / `#site-menu`: focus trap, Escape, `inert` on everything outside, scroll lock, auto-close at ≥1120px — the breakpoint where the pill bar fits its six pills, the switcher, the theme group and the CTA), the `.lang-switch` light-dismiss, same-page scroll-spy, `[data-reveal]` / `[data-reveal="stagger"]` entrance reveals, the hero parallax (`.depth[data-depth]`) and card tilt, the `.copy-mini` buttons, and the live-stats nav badge. Reveals only hide content once `html.js` is set, so the pages are complete without script. Every effect is off under `prefers-reduced-motion`. Keep `THEME_PREPAINT` (from `site-partials.ts`) right after `<body>` on every page — it re-applies the saved override before first paint and is what prevents the light flash on dark-mode visitors.
- **The header, sheet menu and footer are shared**, not hand-copied: `scripts/site-partials.ts` exports `nav(locale, suffix, currentSuffix?, opts?)` (the glass pill bar — brand, `.head-nav` pills into the landing page's sections via `hashPath()` plus Tools, `<details class="lang-switch">`, the `.nm-theme` group, the Connect CTA, the hamburger — and the `#site-menu` sheet) and `footer(locale, currentSuffix?)` (blurb + social icons, the Product / Open source columns, copyright, bottom links, disclaimer), used byte-for-byte by every generator — `gen-index.ts`, `gen-tools.ts`, `gen-legal.ts`, `gen-alternatives.ts`, `gen-login.ts`. `currentSuffix` marks `aria-current="page"`; `opts.dynamicSwitcher` emits the `{{LANG_SWITCHER}}` token for the login template, whose runtime twin is `renderLangSwitcher()` in `src/oauth.ts` (same classes as the static one — `src/oauth.test.ts` pins it). Every visible chrome string comes from `ChromeCopy` (`src/copy/chrome.ts` + one `chrome.<locale>.ts` per locale, read via `chromeFor(locale)`): `nav` holds the short pill labels (How / Examples / Live / Tools / Donate / FAQ), `menu`/`theme`/`landmarks` the sheet and aria strings, `footer` the whole footer shape. `site-partials.ts` also owns `HEAD_ASSETS`, `THEME_PREPAINT`, `SITE_SCRIPT`, `liveBadge()`, `translationNotice()`, `attr()` and the `GITHUB` / `PATREON` / `EMAIL` / `MCP_URL` / `EXT` constants. When the chrome changes, change `site-partials.ts` / `chrome.ts` and re-run every `scripts/gen-*.ts` — see the shared-chrome-data note under "Generators" below.
- **The landing page** (`scripts/gen-index.ts`, `body.landing`) owns only what sits between the chrome: the hero chat, how, connect, onboarding, examples, live board, support, contact, FAQ and closing band. Section ids are `how`, `connect`, `onboarding`, `examples`, `live`, `support`, `contact`, `faq` — `nav()`, `footer()` and `gen-alternatives.ts` link into them through `hashPath()`, so renaming one means changing those too. `LANDING_SCRIPT` (one string, byte-identical on all nine locales, holds no copy) replays the hero chat from the statically rendered thread's `data-add`/`data-clock` attributes, drives the examples picker, polls `/api/stats` every 5 s for the seven stat cards (`total_water_ml` and `weight_lost_g` were added to `public_landing_stats()` for it; both are optional in `LandingStats`, so a DB that lags the app hides the card rather than showing NaN), builds the world map from `/map-data.json` + `timezone_levels` (the tooltip shows the zone name only — levels are the privacy boundary, never counts), renders three Patreon posts from `<template id="patreon-post-tpl">`, and hands the food-log count to site.js's nav badge through the `live-stats` event (`#facts-live` is the marker that stops site.js polling on its own). `src/landing-script.test.ts` pins those hooks; `src/site-copy.test.ts` pins the hero lead's nutrient list and the export/import FAQ answers. Copy lives in `src/copy/index.ts` (`IndexDoc`) plus one `index.<locale>.ts` per locale.
- **The other pages** build from the shared primitives and add only page-scoped rules inline: `/tools` (`body.tools` — glass pill eyebrow with the tool count, a sticky horizontally-scrolling `.nm-chips` category row, one `.nm-panel` per category, tool cards with `<code class="tool-name">` — a test pins that class — and the `.nm-say` "Just say" box); `/alternatives` + the comparison pages (`body.alt` — `.nm-crumb` breadcrumb, tile-card feature grid, the old-app / Nutrition MCP pair with the `.nm-card-dark` emphasis panel, `.nm-steps` install rows with the copy pill, `.nm-faq-row` details, the `.nm-cta-band` closer, the disclaimer as a `.nm-notice`); `/privacy` + `/terms` (`body.legal` — a `.nm-doc` two-column layout: sticky `.nm-toc` that folds into a `<details>` on phones, prose in one `.nm-prose` panel, `legal-section` ids the TOC links to); `/login` (`body.login` — a `.nm-frame` auth card with `.nm-field`/`.nm-input` pills and the `{{ERROR}}` slot rendered as `.error-banner`). `src/alt-pages.test.ts` pins hreflang/canonical, the translation notice, the live badge, the sheet menu, landmarks and the theme group on every generated page; `src/site-copy.test.ts` pins phrases in `tools.html` and the comparison pages.
- **`scripts/depersonalize.ts`** now treats the footer as shared chrome: `CHROME_RULES` (Support/Contact nav links, the footer's Patreon / email / repo sub-links, every repo link, the maintainer handle) run on every page, on top of the landing page's own section and script rules. A rule that reports `⚠ 0×` means the markup drifted from the rule — fix the rule, not the page.
