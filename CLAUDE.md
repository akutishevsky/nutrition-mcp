...
**The landing page (`/` and `/{locale}/`) is on its own design, "Dawn" (Claude Design project `Landing 6a Dawn.dc.html`); every other public page is still on the "Nutrition Facts" system described below.** The landing page loads `public/landing.css` (served at `/landing.css`; tokens on `body.landing`, dark values under both `prefers-color-scheme` and `body.landing[data-theme="dark"]`; Urbanist + Geist Mono) and NOT `/styles.css`. Its header and footer are rendered by `scripts/gen-index.ts` itself rather than `nav()`/`footer()`, but they keep every DOM contract `public/site.js` and `src/alt-pages.test.ts` rely on: `#site-head`, `.head-nav` with `.nav-has-badge` and the `liveBadge()` markup (now exported from `site-partials.ts`), `<details class="lang-switch">` + `.lang-menu[role=group]`, `[data-theme-set]` buttons inside a `.nm-theme[role=group]` segmented control (instead of the `.theme-switch` disclosure), the `#menu-btn` + `#site-menu` sheet below 1120px, and `<nav class="footer-links">`. Section ids are `how`, `connect`, `onboarding`, `examples`, `live`, `support`, `contact`, `faq` — `nav()` and `gen-alternatives.ts` link into them through `hashPath()`, so renaming one means changing those too. `LANDING_SCRIPT` (one string, byte-identical on all nine locales, holds no copy) replays the hero chat from the statically rendered thread's `data-add`/`data-clock` attributes, drives the examples picker, polls `/api/stats` every 5 s for the seven stat cards (`total_water_ml` and `weight_lost_g` were added to `public_landing_stats()` for it; both are optional in `LandingStats`, so a DB that lags the app hides the card rather than showing NaN), builds the world map from `/map-data.json` + `timezone_levels` (the tooltip shows the zone name only — levels are the privacy boundary, never counts), renders three Patreon posts from `<template id="patreon-post-tpl">`, and hands the food-log count to site.js's nav badge through the `live-stats` event (`#facts-live` is the marker that stops site.js polling on its own). `src/landing-script.test.ts` pins those hooks; `src/site-copy.test.ts` pins the hero lead's nutrient list and the export/import FAQ answers. Light-mode `--acc` and `--ink3` deliberately differ from the design file for AA contrast — see the token block. Copy lives in `src/copy/index.ts` (`IndexDoc`) plus one `index.<locale>.ts` per locale, the same pattern as everything else.

The rest of the site shares one design system, "Nutrition Facts": the FDA label's grammar — a heavy 3px `var(--rule)` above every section head, hairline rules between rows, bold label left / figure right, ruled 1px-gap grids instead of floating cards — is the structural motif on every page. (The `.facts` / `.odo-*` rules still in `styles.css` were the old landing page's stats panel and odometer; nothing renders them any more.) Type is Bricolage Grotesque (display, `.section-title` / `.display`), Instrument Sans (body) and Geist Mono (eyebrows, URLs, tool names), all from Google Fonts; the CSP already allows them. Light is paper white on green-black ink; dark is forest-tinted, not grey.
