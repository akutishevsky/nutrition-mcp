...
import { ALT_PAGES, LOCALES, PAGE_ROUTES, SITE_LOCALES } from "./routes.js";
...
// The landing page's in-chat widget cards, generated by
// scripts/gen-widget-card.ts: the widget's own CSS scoped to `.nm-widget-card`,
// and the interaction runtime (the same shared partials the widgets are
// assembled from, minus the MCP host bridge) that makes the build-time-rendered
// cards live. Served beside /styles.css and /site.js and with the same headers,
// but loaded ONLY by the landing page — they are not in HEAD_ASSETS.
app.get("/widget-card.css", async (c) => {
    const file = Bun.file("./public/widget-card.css");
    return c.body(await file.text(), 200, { "Content-Type": "text/css" });
});
app.get("/widget-card.js", async (c) => {
    const file = Bun.file("./public/widget-card.js");
    return c.body(await file.text(), 200, {
        "Content-Type": "text/javascript; charset=utf-8",
    });
});
// One tiny strings file per locale beside it, so the runtime above stays one
// cacheable file across all nine locale pages instead of nine bundles carrying
// the same code with a different dictionary baked in. A route per locale rather
// than a path parameter: the locale set is fixed and known, so an unknown one is
// a 404 from the router and never reaches a file read.
for (const locale of SITE_LOCALES) {
    app.get(`/widget-card.${locale}.js`, async (c) => {
        const file = Bun.file(`./public/widget-card.${locale}.js`);
        return c.body(await file.text(), 200, {
            "Content-Type": "text/javascript; charset=utf-8",
        });
    });
